...
from minimal import Crossbar, Gate, GateType, Operation
...
def MultPIM(crossbar: Crossbar, N: int):
    ...
    TEMP2 = 6
    TEMP3 = 7
    NOTBROADCAST = 8  # stores an indicator for if the partition receives not in broadcast
    ...
    crossbar.perform(
        Operation([Gate(GateType.INIT1, [], [(1, TEMP1), (1, ABIT), (1, BBIT)])] + [Gate(GateType.INIT1, [],
                  [(partition, ABIT), (partition, BBIT), (partition, ABBIT),
                   (partition, TEMP1), (partition, TEMP2), (partition, TEMP3), (partition, NOTBROADCAST)])
    ...
    # Store not-broadcast in the partitions (using bulk INIT)
    is_notted = [False] + [bool(bin(p).count('1') % 2 == 0) for p in range(N)]
    crossbar.perform(Operation([Gate(GateType.INIT0, [], [(p, NOTBROADCAST)]) for p in range(2, N + 1) if not is_notted[p]]))
    ...
    for k in range(2 * N):
        ...
        if k < N:
            ...
            # --- log_2(N) OPs --- #
            crossbar.perform(Operation([Gate(GateType.MAGIC_NOT, [(0, N + k)], [(1, BBIT)])]))
            ...
            # Compute XNOR(BBIT, NOTBROADCAST)
            crossbar.perform(Operation(
                [Gate(GateType.MAGIC_NOR, [(p, BBIT), (p, NOTBROADCAST)], [(p, TEMP1)])
                 for p in range(2, N + 1)
            ]))
            crossbar.perform(Operation(
                [Gate(GateType.MAGIC_NOR, [(p, BBIT), (p, TEMP1)], [(p, TEMP2)])
                 for p in range(2, N + 1)
            ]))
            crossbar.perform(Operation(
                [Gate(GateType.MAGIC_NOR, [(p, NOTBROADCAST), (p, TEMP1)], [(p, TEMP3)])
                 for p in range(2, N + 1)
            ]))
            crossbar.perform(Operation(
                [Gate(GateType.INIT1, [], [(p, BBIT)])
                 for p in range(2, N + 1)
            ]))
            crossbar.perform(Operation(
                [Gate(GateType.MAGIC_NOR, [(p, TEMP2), (p, TEMP3)], [(p, BBIT)])
                 for p in range(2, N + 1)
            ]))
            crossbar.perform(Operation(
                [Gate(GateType.INIT1, [], [(p, TEMP1), (p, TEMP2), (p, TEMP3)])
                 for p in range(2, N + 1)
            ]))
            crossbar.perform(Operation([
                Gate(GateType.MAGIC_NOR, [(j, ABIT), (j, BBIT)], [(j, ABBIT)])
            ...
            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.MAGIC_NOR, [(j, ABBIT), (j, SBIT)], [(j, TEMP1)])
                for j in range(2, N + 1)
            ]))
            # --- 1 OP --- #
            crossbar.perform(Operation([
                Gate(GateType.MAGIC_NOR, [(j, ABBIT), (j, TEMP1)], [(j, TEMP2)])
                for j in range(2, N + 1)
            ]))
            # --- 1 OP --- #
            crossbar.perform(Operation([
            ...
            crossbar.perform(Operation([Gate(GateType.MAGIC_NOR, [(1, ABIT), (1, BBIT)], [(2, SBIT)])]))
            crossbar.perform(Operation([
                Gate(GateType.MAGIC_NOR, [(j, TEMP2), (j, TEMP3)], [(j + 1, SBIT)])
                for j in range(2, N, 2)
            ]))
            crossbar.perform(Operation([Gate(GateType.MAGIC_NOR, [(N, TEMP2), (N, TEMP3)], [(N + 1, k)])]))
            ...
            crossbar.perform(Operation([Gate(GateType.MAGIC_NOR, [(N, CBIT), (N, TEMP1)], [(N + 1, k)])]))
...
# Crossbar
partitionSize = 9
crossbar = Crossbar([2 * N] + [6] + [partitionSize] * (N - 1) + [2 * N])
