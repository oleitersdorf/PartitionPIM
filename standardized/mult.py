...
from standardized import Crossbar, Gate, GateType, Operation
...
def MultPIM(crossbar: Crossbar, N: int):
    ...
    for k in range(2 * N):
        ...
        if k < N:
            ...
            is_notted[1:] = [bool(bin(p).count('1') % 2 == 0) for p in range(N)]

            # Compute partial products
            # --- 4 OPs --- #
            crossbar.perform(Operation(
                [Gate(GateType.MAGIC_NOT, [(p, BBIT)], [(p, TEMP1)])
                 for p in range(1, N + 1) if not is_notted[p]
            ]))
            crossbar.perform(Operation(
                [Gate(GateType.MAGIC_NOR, [(p, ABIT), (p, TEMP1)], [(p, ABBIT)])
                 for p in range(1, N + 1) if not is_notted[p]
            ]))
            crossbar.perform(Operation(
                [Gate(GateType.INIT1, [], [(p, TEMP1)])
                 for p in range(1, N + 1) if not is_notted[p]
            ]))
            crossbar.perform(Operation([
